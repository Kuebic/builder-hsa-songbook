...
const mockFormatter = {
  format: vi.fn(() => '<div class="chord-line"><span class="chord">C</span> <span class="lyrics">Amazing grace</span></div>'),
};
...
describe("ChordDisplay Component", () => {
  ...
  describe("Basic Rendering", () => {
    it("renders chord content correctly", () => {
      ...
      expect(screen.getByRole("region", { name: /chord chart/i })).toBeInTheDocument();
  ...
  describe("Controls Integration", () => {
    ...
    it("hides controls when showControls is false", () => {
      ...
      expect(screen.queryByTestId("chord-display-controls")).not.toBeInTheDocument();
    });
  ...
  describe("Empty States", () => {
    it("displays empty state for no content", () => {
      ...
      expect(screen.getByText(/add chordpro formatted lyrics/i)).toBeInTheDocument();
    });

    it("handles whitespace-only content as parsing error", async () => {
      // Mock parser hook to treat whitespace as invalid
      const { useChordSheetParser } = await import("../../hooks/useChordSheetParser");
      ...
      expect(screen.getByText("Unable to parse chord content")).toBeInTheDocument();
    });
  });
  ...
  describe("Accessibility", () => {
    it("has proper ARIA attributes", () => {
      ...
      expect(chordRegion).toHaveAttribute("aria-describedby", "chord-display-instructions");
    ...
    it("provides screen reader instructions", () => {
      ...
      const instructions = document.getElementById("chord-display-instructions");
    ...
    it("includes transposition info in screen reader instructions", async () => {
      ...
      const { useChordTransposition } = await import("../../hooks/useChordTransposition");
  ...
  describe("Performance", () => {
    ...
    it("re-renders when content changes", () => {
      ...
      rerender(<ChordDisplay {...defaultProps} content="{title: New Song}\n[G]Different content" />);
  ...
  describe("Integration", () => {
    ...
    it("handles transposition correctly", async () => {
      const { useChordTransposition } = await import("../../hooks/useChordTransposition");
      const mockTransposition = vi.mocked(useChordTransposition).mockReturnValue({
