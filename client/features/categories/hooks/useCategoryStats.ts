    ...
    queryFn: async (): Promise<CategoryStats[]> => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/categories/stats?${searchParams.toString()}`,
          {
        ...
        if (!response.ok) {
          ...
          if (response.status >= 500) {
            ...
            const { generateMockCategoryStats } = await import(
              "../utils/categoryHelpers"
            );
            return generateMockCategoryStats();
          }
          throw new Error(
            `Failed to fetch category stats: ${response.statusText}`,
          );
        }
        ...
        if (!result.success) {
          throw new Error(
            result.error?.message || "Failed to fetch category stats",
          );
        }
...
export function useCategoryStatsById(categoryId: string) {
  ...
  const categoryStats =
    allStats?.find((stat) => stat.id === categoryId) || null;
...
export function useCategoriesWithRecentActivity() {
  ...
  const categoriesWithActivity =
    allStats?.filter((stat) => stat.recentCount > 0) || [];
