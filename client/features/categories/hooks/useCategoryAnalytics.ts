...
import {
  ...
  hasRecentActivity,
} from "@features/categories/utils/categoryHelpers";
...
export function useCategoryRecommendations(
  ...
  const recommendations = useMemo(() => {
    ...
    return getCategoryRecommendations(userFavoriteThemes, categoryStats).slice(
      0,
      limit,
    );
...
export function useCategoryComparison(categoryIds: string[]) {
  ...
  const comparisonData = useMemo(() => {
    if (!allStats || categoryIds.length === 0) {
      return [];
    }

    const selectedCategories = allStats.filter((stat) =>
      categoryIds.includes(stat.id),
    );
    ...
    return selectedCategories.map((category) => ({
      ...
      metrics: {
        songsPerRating:
          category.songCount > 0 ? category.avgRating / category.songCount : 0,
        activityRatio:
          category.songCount > 0
            ? category.recentCount / category.songCount
            : 0,
        popularityRank:
          allStats.findIndex((stat) => stat.id === category.id) + 1,
        isAboveAverage: {
          songCount:
            category.songCount >
            allStats.reduce((sum, s) => sum + s.songCount, 0) / allStats.length,
          rating:
            category.avgRating >
            allStats.reduce((sum, s) => sum + s.avgRating, 0) / allStats.length,
          popularity:
...
export function useCategoryHealth() {
  ...
  const healthMetrics = useMemo(() => {
    ...
    const trendingCategories = categoryStats.filter((cat) =>
      hasRecentActivity(cat),
    ).length;
    const highRatedCategories = categoryStats.filter(
      (cat) => cat.avgRating > 4.0,
    ).length;
    const lowContentCategories = categoryStats.filter(
      (cat) => cat.songCount < 5,
    ).length;
    ...
    return {
      overall: {
        score: Math.round(
          ((activeCategories / totalCategories) * 0.4 +
            (trendingCategories / totalCategories) * 0.3 +
            (highRatedCategories / totalCategories) * 0.3) *
            100,
        ),
        status:
          activeCategories / totalCategories > 0.8
            ? "excellent"
            : activeCategories / totalCategories > 0.6
              ? "good"
              : activeCategories / totalCategories > 0.4
                ? "fair"
                : "needs_attention",
      ...
      metrics: {
        ...
        contentGapRate: Math.round(
          (lowContentCategories / totalCategories) * 100,
        ),
      },
      recommendations: [
        ...
        popularSearches: [
          {
            query: "traditional",
            count: 156,
            categories: ["traditional-holy", "classic-hymns"],
          },
          {
            query: "contemporary",
            count: 89,
            categories: ["contemporary-christian"],
          },
          { query: "pioneer", count: 67, categories: ["american-pioneer"] },
          {
            query: "holy",
            count: 234,
            categories: ["traditional-holy", "new-holy"],
          },
        ],
        categoryDiscovery: {
