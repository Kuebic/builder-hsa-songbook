...
export function sortCategories(
  ...
  switch (sortBy) {
    case 'popular':
      return [...categories].sort((a, b) => b.popularityScore - a.popularityScore);
    case 'alphabetical':
...
export function assignClientSongCategories(song: ClientSong): { categories: string[], scores: Record<string, number> } {
...
export function enhanceSongsWithCategories(songs: ClientSong[]): CategorizedSong[] {
...
function inferUsageContext(song: ClientSong): CategorizedSong['usageContext'] {
  ...
  if (themes.some(t => t.toLowerCase().includes('opening')) || 
      title.includes('welcome') || title.includes('gather')) {
    return 'opening';
  }
  
  if (themes.some(t => t.toLowerCase().includes('communion')) ||
      title.includes('bread') || title.includes('wine') || title.includes('table')) {
    return 'communion';
  }
  
  if (themes.some(t => t.toLowerCase().includes('closing')) ||
      title.includes('send') || title.includes('go forth') || title.includes('blessing')) {
    return 'closing';
  }
  
  if (themes.some(t => ['seasonal', 'christmas', 'easter', 'wedding'].includes(t.toLowerCase()))) {
    return 'special';
  }
...
export function generateCategoryInsights(categories: CategoryStats[]): {
  ...
  const mostRated = categories.reduce((prev, current) => 
...
export function formatCategoryStats(category: CategoryStats): {
  ...
  const formattedRating = category.avgRating > 0 ? category.avgRating.toFixed(1) : 'N/A';
  const formattedPopularity = category.popularityScore.toFixed(1);
  
  const recentLabel = category.recentCount === 0 ? 'No recent additions' :
                     category.recentCount === 1 ? '1 recent addition' :
                     `${category.recentCount} recent additions`;
  
  const songCountLabel = category.songCount === 1 ? '1 song' : `${category.songCount} songs`;
...
export function generateMockCategoryStats(): CategoryStats[] {
  return SPIRITUAL_CATEGORIES.map((category, index) => ({
    ...
    topSongs: [
      { id: `song-${index}-1`, title: `Popular Song ${index + 1}`, artist: `Artist ${index + 1}` },
      { id: `song-${index}-2`, title: `Beloved Hymn ${index + 1}`, artist: `Composer ${index + 1}` },
    ],
...
export function hasRecentActivity(category: CategoryStats, daysThreshold: number = 7): boolean {
...
export function getCategoryRecommendations(
  ...
  const scoredCategories = categories.map(category => {
    ...
    if (!spiritualCategory) return { ...category, recommendationScore: 0 };
