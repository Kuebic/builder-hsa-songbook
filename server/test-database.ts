...
class DatabaseTester {
  ...
  async testUserSchemaOperations(): Promise<void> {
    await this.runTest("User Schema CRUD", async () => {
      ...
      if (updatedUser.stats.songsCreated !== 1) {
        throw new Error("Song count not incremented");
      }
  ...
  async testSongSchemaOperations(): Promise<void> {
    await this.runTest("Song Schema CRUD", async () => {
      ...
      // Test compression - Note: chordData has moved to Arrangement model
      // @ts-ignore - Old test for when chordData was on Song
      if (!Buffer.isBuffer(savedSong.chordData)) {
        throw new Error("Chord data not compressed to Buffer");
      }

      // Test decompression method
      // @ts-ignore - Old test method
      ...
      if (updatedSong.metadata.views !== 1) {
        throw new Error("Views not incremented");
      }
      if (updatedSong.metadata.ratings.average !== 4.5) {
  ...
  async testArrangementSchemaOperations(): Promise<void> {
    await this.runTest("Arrangement Schema CRUD", async () => {
      ...
      if (!user || !song) {
        throw new Error("Test user or song not found");
      }
  ...
  async testSetlistSchemaOperations(): Promise<void> {
    await this.runTest("Setlist Schema CRUD", async () => {
      ...
      if (!updatedSetlist) {
        throw new Error("Setlist not found after update");
      }
      if (updatedSetlist.songs.length !== 2) {
        throw new Error("Song not added to setlist");
      }
      if (updatedSetlist.metadata.usageCount !== 1) {
        throw new Error("Usage count not incremented");
      }
      if (updatedSetlist.metadata.estimatedDuration !== 8) {
        throw new Error("Duration not recalculated");
      }
  ...
  async testSearchAndIndexes(): Promise<void> {
    await this.runTest("Search & Index Functionality", async () => {
      ...
      const arrangementSearchResults = await Arrangement.searchArrangements(
        "acoustic",
        10,
      );
      ...
      // @ts-ignore - Method doesn't exist on current Song model
  ...
  async testStorageOptimization(): Promise<void> {
    await this.runTest("Storage Optimization", async () => {
      ...
      const avgSongSize =
        songs.reduce((sum, song) => sum + song.documentSize, 0) / songs.length;
