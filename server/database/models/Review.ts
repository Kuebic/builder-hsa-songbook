...
export interface IReviewModel extends Model<IReview> {
  findByArrangement(arrangementId: string | Types.ObjectId, includeReported?: boolean): Promise<IReview[]>;
  findByUser(userId: string | Types.ObjectId): Promise<IReview[]>;
  findUserReview(arrangementId: string | Types.ObjectId, userId: string | Types.ObjectId): Promise<IReview | null>;
  getAverageRating(arrangementId: string | Types.ObjectId): Promise<{ average: number; count: number }>;
...
reviewSchema.methods.unmarkAsHelpful = async function(userId: Types.ObjectId) {
  this.helpful = this.helpful.filter((id: Types.ObjectId) => !id.equals(userId));
  return await this.save();
};
...
reviewSchema.statics.findUserReview = async function(
  ...
  return await this.findOne({ arrangementId, userId })
    .populate("userId", "name email");
};
...
reviewSchema.statics.findReported = async function(limit: number = 50): Promise<IReview[]> {
