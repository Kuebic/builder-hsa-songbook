...
export interface IVerseModel extends Model<IVerse> {
  findBySong(songId: string | Types.ObjectId, status?: VerseStatus): Promise<IVerse[]>;
...
verseSchema.methods.removeUpvote = async function(userId: Types.ObjectId) {
  this.upvotes = this.upvotes.filter((id: Types.ObjectId) => !id.equals(userId));
  return await this.save();
};
...
verseSchema.statics.findPending = async function(limit: number = 50): Promise<IVerse[]> {
