...
const CATEGORY_INDEXES: IndexDefinition[] = [
  {
    ...
    options: {
      background: true,
      comment:
        "Primary index for category assignment and public song filtering",
    },
...
async function checkCategoryIndexes(): Promise<void> {
  ...
  try {
    ...
    const presentIndexes = requiredIndexNames.filter((name) =>
      existingIndexNames.includes(name),
    );

    console.log("\n🔍 Category Index Status Check");
    console.log("=============================");
    console.log(
      `✅ Present: ${presentIndexes.length}/${requiredIndexNames.length} indexes`,
    );
...
if (require.main === module) {
  ...
  switch (command) {
    ...
    default:
      ...
      console.log("Usage:");
      console.log(
        "  npx tsx server/database/scripts/createCategoryIndexes.ts create",
      );
      console.log(
        "  npx tsx server/database/scripts/createCategoryIndexes.ts check",
      );
      console.log(
        "  npx tsx server/database/scripts/createCategoryIndexes.ts drop",
      );
      break;
  }
}
